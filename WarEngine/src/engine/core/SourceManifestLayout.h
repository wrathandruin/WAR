    ...
    public:
            ...
            const char* manifestFileName)
        {
            ...
            const std::filesystem::path sourceManifestPath =
                runtimeBoundaryReport.repoRoot / "Project" / "SourceManifests" / packagedLane / manifestFileName;
            const std::filesystem::path legacyRepoManifestPath =
            ...
            error.clear();
            if (std::filesystem::exists(legacyRepoManifestPath, error)
                && std::filesystem::is_regular_file(legacyRepoManifestPath, error))
            {
                return legacyRepoManifestPath;
            }
