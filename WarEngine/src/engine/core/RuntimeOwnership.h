...
namespace war
{
    ...
    public:
            ...
            const std::string& preferredSlotName = "primary")
        {
            ...
            {
                report.issues.push_back("Deployable Project/Environment root overlaps with mutable runtime state.");
            }

            if (!report.primarySavePathOwned)
            {
                report.issues.push_back("Primary save path is not contained under Project/Runtime/Saves.");
            }
