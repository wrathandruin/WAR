...
namespace war
{
    ...
    RuntimeBoundaryReport RuntimePaths::buildReport()
    {
        ...
        const bool packagedMarkersPresent =
            ...
            || fileExists(report.executableDirectory / "session_mvp_candidate_manifest.txt")
        ...
        const std::filesystem::path sourceAssetRoot = report.repoRootResolved
            ? (directoryExists(report.repoRoot / "Assets")
                ? report.repoRoot / "Assets"
                : report.repoRoot / "assets")
            : std::filesystem::path{};
        const std::filesystem::path sourceProjectRoot = report.repoRootResolved
            ? report.repoRoot / "Project"
            : std::filesystem::path{};
        const std::filesystem::path sourceRuntimeRoot = report.repoRootResolved
            ? sourceProjectRoot / "Runtime"
            : std::filesystem::path{};
        const std::filesystem::path legacySourceRuntimeRoot = report.repoRootResolved
        ...
        else
        {
            report.runtimeRoot = packagedLayoutDetected
                ? packagedRuntimeRoot
                : (report.repoRootResolved
                    ? (directoryExists(sourceRuntimeRoot) ? sourceRuntimeRoot : legacySourceRuntimeRoot)
        ...
        const std::filesystem::path sourceEnvironmentRoot = report.repoRootResolved
            ? sourceProjectRoot / "Environment"
            : std::filesystem::path{};
        const std::filesystem::path legacySourceEnvironmentRoot = report.repoRootResolved
        ...
        else if (report.repoRootResolved && directoryExists(legacySourceEnvironmentRoot))
        {
            report.environmentRoot = legacySourceEnvironmentRoot;
            report.environmentRootResolved = true;
        }
    ...
    bool RuntimePaths::hasRepoMarkers(const std::filesystem::path& candidate)
    {
        const bool canonicalLayout =
            fileExists(candidate / "WAR.sln")
            && directoryExists(candidate / "Assets")
            && directoryExists(candidate / "Docs")
            && directoryExists(candidate / "Project")
            && directoryExists(candidate / "WarEngine");

        const bool legacyLayout =
            directoryExists(candidate / "src")
            && directoryExists(candidate / "assets")
            && directoryExists(candidate / "Docs")
            && fileExists(candidate / "README.md");
