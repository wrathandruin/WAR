...
set "MANIFEST_PATH=%STAGE_ROOT%\beta_release_candidate_manifest.txt"

if not exist "%SOLUTION_FILE%" (
    echo [M49] ERROR: WAR.sln not found at "%SOLUTION_FILE%".
    exit /b 1
)

set "VSWHERE_EXE=%ProgramFiles(x86)%\Microsoft Visual Studio\Installer\vswhere.exe"
if not exist "%VSWHERE_EXE%" (
    echo [M49] ERROR: vswhere.exe not found at "%VSWHERE_EXE%".
    exit /b 1
)
...
if not defined MSBUILD_EXE (
    echo [M49] ERROR: MSBuild.exe could not be resolved via vswhere.exe.
    exit /b 1
)

echo [M49] Rebuilding WAR %CONFIG%^|%PLATFORM%...
"%MSBUILD_EXE%" "%SOLUTION_FILE%" /m /nologo /t:Rebuild /p:Configuration=%CONFIG%;Platform=%PLATFORM%
if errorlevel 1 (
    echo [M49] ERROR: build failed.
    exit /b 1
)

if not exist "%CLIENT_EXE_PATH%" (
    echo [M49] ERROR: expected client executable missing at "%CLIENT_EXE_PATH%".
    exit /b 1
)

if not exist "%SERVER_EXE_PATH%" (
    echo [M49] ERROR: expected host executable missing at "%SERVER_EXE_PATH%".
    exit /b 1
)
...
if not exist "%STAGE_ROOT%\assets\shaders\dx11\vs_color.bin" (
    echo [M49] ERROR: staged shader binary missing at "%STAGE_ROOT%\assets\shaders\dx11\vs_color.bin".
    exit /b 1
)

if not exist "%STAGE_ROOT%\assets\shaders\dx11\fs_color.bin" (
    echo [M49] ERROR: staged shader binary missing at "%STAGE_ROOT%\assets\shaders\dx11\fs_color.bin".
    exit /b 1
)

if not exist "%STAGE_ROOT%\assets\shaders\dx11\vs_texture.bin" (
    echo [M49] ERROR: staged shader binary missing at "%STAGE_ROOT%\assets\shaders\dx11\vs_texture.bin".
    exit /b 1
)

if not exist "%STAGE_ROOT%\assets\shaders\dx11\fs_texture.bin" (
    echo [M49] ERROR: staged shader binary missing at "%STAGE_ROOT%\assets\shaders\dx11\fs_texture.bin".
    exit /b 1
)

if not exist "%STAGE_ROOT%\assets\textures\world_atlas.bmp" (
    echo [M49] ERROR: staged atlas texture missing at "%STAGE_ROOT%\assets\textures\world_atlas.bmp".
    exit /b 1
)

if not exist "%STAGE_ROOT%\BetaContent\m49_beta_content_manifest.txt" (
    echo [M49] ERROR: staged beta content manifest missing at "%STAGE_ROOT%\BetaContent\m49_beta_content_manifest.txt".
    exit /b 1
)

if not exist "%STAGE_ROOT%\BetaContent\m49_retention_journey_cards.txt" (
    echo [M49] ERROR: staged retention journey cards missing at "%STAGE_ROOT%\BetaContent\m49_retention_journey_cards.txt".
    exit /b 1
)
...
(
    echo WAR Beta Release Candidate Manifest
    echo Milestone: M49 - Beta Content Scale / Retention Expansion / Release-Candidate Discipline
    ...
    echo - validate_m49_beta_candidate_package_win64.bat
) > "%MANIFEST_PATH%"

echo [M49] Beta release candidate package staged at "%STAGE_ROOT%".
exit /b 0
...
if errorlevel 8 (
    echo [M49] ERROR: failed to stage !COPY_LABEL! from "!COPY_SOURCE!" to "!COPY_DEST!" - robocopy exit !ROBOCOPY_EXIT!.
    exit /b 1
)
...
if %STAGE_ATTEMPT% GEQ 10 (
    echo [M49] ERROR: unable to clear existing stage root "%STAGE_ROOT%".
    exit /b 1
)
taskkill /IM WAR.exe /F >nul 2>nul
taskkill /IM WARServer.exe /F >nul 2>nul
