...
set "RUNTIME_ROOT=%PACKAGE_ROOT%\M50InstallerRuntime"
set "CONFIG_DIR=%RUNTIME_ROOT%\Config"
set "LOG_DIR=%RUNTIME_ROOT%\Logs"
set "INSTALLER_DIR=%LOG_DIR%\Installer"
set "MANIFEST_FILE=%INSTALLER_DIR%\installer_runtime_manifest.txt"
set "REPORT_PATH=%LOG_DIR%\m50_installer_validation.txt"
set "DETAILS_PATH=%LOG_DIR%\m50_installer_validation_details.txt"
...
call :wait_for_file "%MANIFEST_FILE%" "Installer runtime manifest"
if "%FAILED%"=="1" goto :cleanup

findstr /b /c:"version=1" "%MANIFEST_FILE%" >nul || set "FAILED=1"
findstr /b /c:"installer_manifest_path=" "%MANIFEST_FILE%" >nul || set "FAILED=1"
findstr /b /c:"component=host-bootstrap" "%MANIFEST_FILE%" >nul || set "FAILED=1"
...
(
    echo WAR M50 Installer Productionization Validation
