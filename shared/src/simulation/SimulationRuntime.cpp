...
namespace war
{
        ...
        std::string& outCorrectionReason)
    {
        ...
        if (positionDistance > correctionTolerance)
        {
            corrected = true;
        ...
        if (m_playerActorState.health != snapshot.playerActorState.health
        ...
        if (m_combatEncounterState.active != snapshot.combatEncounterState.active
        ...
        if (!loadPersistedState(snapshot, loadError, false))
        {
