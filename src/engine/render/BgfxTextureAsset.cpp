...
#pragma pack(pop)
        ...
        bool loadBmpPixels(const std::string& path, BmpPixels& outPixels, std::string& outStatus)
        {
    ...
    bool BgfxTextureAsset::loadFromBmpFile(const std::string& path, std::string& outStatus)
    {
#if WAR_HAS_BGFX
        if (m_attemptedLoad)
        {
        ...
        BmpPixels pixels{};
        if (!loadBmpPixels(path, pixels, m_lastStatus))
        {
            outStatus = m_lastStatus;
            return false;
        }
